...
public partial class SearchInput
{
    ...
    // see its doc comment); .wss-icon-spin (wss-controls.css, reusing the existing wss-msg-spin
    // keyframe rather than defining a second one) is what actually rotates it. The wrapped form lives
    // beside the glyph itself, so Table's loading mask and this button share one copy.
