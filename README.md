...
**Breaking**
- **The AntD-style calendar-dropdown date control, formerly `EditDatePicker<T>`, is renamed to `EditDate<T>` and is now the default date control.** The previous native-`<input>` `EditDate<T>` is renamed to `EditDateNative<T>`. A compile-time `[Obsolete(error: true)]` `EditDatePicker<T>` stub (inheriting `EditDate<T>`) catches stale `<EditDatePicker>` markup with an actionable error instead of silently binding to whatever `EditDate` now means. **A bare `<EditDate>` reference that meant the old native input still compiles after upgrading — it silently renders the calendar dropdown instead, since the class name now means something different.** Audit any existing `<EditDate>` usage before upgrading: keep it as `<EditDate>` if you actually wanted the calendar dropdown (nothing to change beyond confirming that), or switch it to `<EditDateNative>` if you specifically need the native `<input type="date">` behavior — both controls support the identical set of bound types and `Type` values, so the parameters carry over unchanged either way. Stale `<EditDatePicker>` markup fails the build with a clear message pointing at `<EditDate>`.
