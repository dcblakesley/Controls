...
- **`EditDateNative`** - Native `<input type="date">` (or `datetime-local`/`month`/`time`, per `Type`) date field, zero JS, styled entirely by `edit-controls.css`. New in 10.7.0: `Min`/`Max` (`DateTime?`, same shape as `EditDate`'s — its first bounds support ever) render the native input's own `min`/`max` attribute formatted to match `Type`, omitted entirely in `Time` mode for parity with `EditDate`, and fall back to the bound property's `[MinValue]`/`[MaxValue]`, then `[Range]`, when unset — see [Model-declared Min/Max](#model-declared-minmax-minvaluemaxvalue). `Size` behaves the same as `EditString`'s, though `EditDateNative` never enters affix mode itself (no `Prefix`/`Suffix`/`AllowClear`/etc.), so only the input itself carries the size class
