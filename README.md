...
- **`Table<TItem>` gains seven "Override to localize" label parameters, covering the last user-facing strings that had no override:** `SelectRowLabel` (`"Select row"`), `SelectAllRowsLabel` (`"Select all rows"`), `SelectAllRowsOnPageLabel` (`"Select all rows on this page"` — used instead of `SelectAllRowsLabel` whenever the table is paged), `PaginationLabel` (`"Pagination"`), `TopPaginationLabel`/`BottomPaginationLabel` (`"Pagination (top)"`/`"Pagination (bottom)"`, used only when `PagerPosition="Both"` so the two pagers stay distinguishable), and `SortLabel` (`"Sort"` — the sort button's accessible name on a column with no `Title`). Previously a fully localized table still announced its own selection checkboxes, pagers and sort buttons in English. Every default renders byte-identically to the literal it replaced, so nothing changes unless you set one. One string is still hardcoded — a filter button on a column with no `Title` falls back to `"Filter"` with no override.
...
- **`Tabs`: a conditionally-rendered tab inserted *before* tabs Blazor skipped now takes its declared position.** A content-less tab whose primitive parameters didn't change is skipped by the render-tree diff and never re-registers, so the strip re-inserted it at its old index and pushed a newly-declared first tab behind it — permanently, since nothing later corrects it. Declared `[d(new), a, b, c]` rendered as `[a, b, c, d]`. The newcomer's position genuinely isn't in the registration data (a prepend and an append leave identical state), so the strip now asks for one clean re-registration pass in document order, bounded to a single rebuild and only when a structural insertion coincided with skipped tabs. **Documented limitation:** that pass replaces the whole `ChildContent` subtree, so a *non-`Tab`* component you declared inside `<Tabs>` is torn down with it and loses its instance state (cached lookups, element references, subscriptions, timers). It never happens on an ordinary re-render or on a tab removal; if you have such a component, declare it inside a tab's own pane content or as a sibling of `<Tabs>`. Keyboard navigation across the rebuild is preserved (the nav buttons are keyed to their tabs so their element references are re-captured).
