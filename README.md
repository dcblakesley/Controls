...
- **Security: `EditString`'s read-only link mode now strips ASCII tab/CR/LF before checking the URL scheme, closing a `javascript:` bypass of the http/https/mailto allow-list.** `Uri.TryCreate(Url, UriKind.Absolute, ...)` fails to parse a URL carrying a tab/newline inside or right after its scheme (e.g. `java<TAB>script:alert(1)`), and the old code treated anything unparseable as a safe relative URL, rendering it into `href` verbatim. A browser's own URL parser strips those characters before parsing, then re-forms and runs the `javascript:` URL on click — `SafeUrl` now strips them first, so the allow-list check sees exactly what the browser will see, and renders the stripped value (never the raw `Url`) when it passes.
