...
- **`DatePicker`** - Single-value field with a calendar suffix opening a dropdown panel. Bind with `@bind-Value` (`DateTime?`); `Mode` (`DatePickerMode`: `Date` default — a one-month calendar with a month/year quick-select header; `Month` — a year header over a 3x4 month-button grid; `Time` — hour/minute/second selects over an OK button; `DateTime` — the day calendar with that same time row and OK button appended below it) selects the panel and the commit-time normalization (`Date` keeps the date, `Month` normalizes to the 1st of the month at midnight, `DateTime` truncates to whole seconds, `Time` anchors to `DateTime.Today` plus the time-of-day). Picking a day/month (or typing text + Enter) commits and closes; in `Time`/`DateTime` mode the time selects — and, in `DateTime` mode, a day click — commit immediately without closing, since the user may still want to adjust the other part, and the new small primary OK button (`wss-picker-ok`) is the close signal instead; `Min` / `Max` (checked at month granularity in `Month` mode, date granularity in `DateTime` mode, ignored entirely in `Time` mode), `Format` / `Placeholder` (both `string?`, null picks `Mode`'s default — `MM/dd/yyyy`/"Select date" for `Date`, `MM/yyyy`/"Select month" for `Month`, `MM/dd/yyyy HH:mm:ss`/"Select date" for `DateTime`, `HH:mm:ss`/"Select time" for `Time`), `AllowClear`, `Width`, `FirstDayOfWeek` (`Date`/`DateTime` modes only), `HourSelectLabel`/`MinuteSelectLabel`/`SecondSelectLabel` (accessible names for the time selects), `OkText` (default "OK") — the single-value sibling of `DateRangePicker`, sharing its calendar internals and outside-click/Escape close behavior
