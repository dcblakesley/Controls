...
- **`DatePicker`** - Single-value field with a calendar suffix opening a dropdown panel. Bind with `@bind-Value` (`DateTime?`); `Mode` (`DatePickerMode`: `Date` default — a one-month calendar with a month/year quick-select header, or `Month` — a year header over a 3x4 month-button grid; `DateTime`/`Time` exist on the enum for a follow-up release) selects the panel and the commit-time normalization (`Date` keeps the date, `Month` normalizes to the 1st of the month at midnight). Picking a day/month (or typing text + Enter) commits and closes; `Min` / `Max` (checked at month granularity in `Month` mode), `Format` / `Placeholder` (both `string?`, null picks `Mode`'s default — `MM/dd/yyyy`/"Select date" for `Date`, `MM/yyyy`/"Select month" for `Month`), `AllowClear`, `Width`, `FirstDayOfWeek` (`Date` mode only) — the single-value sibling of `DateRangePicker`, sharing its calendar internals and outside-click/Escape close behavior
