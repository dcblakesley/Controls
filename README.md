...
- **`Tabs`: a conditionally-rendered tab inserted *before* tabs Blazor skipped now takes its declared position — each `Tab` renders its own nav button.** A content-less tab whose primitive parameters didn't change is skipped by the render-tree diff and never re-registers, so the old strip (which rendered the buttons itself from collected registrations) re-inserted it at its old index and pushed a newly-declared first tab behind it — permanently. Declared `[d(new), a, b, c]` rendered as `[a, b, c, d]`. Now the tab's button lives in the `Tab`'s own render tree inside the tablist, so the render-tree diff places it in document order with no registration bookkeeping to go stale, and a non-`Tab` component declared inside `<Tabs>` keeps its instance state across such an insertion (no rebuild pass exists to tear it down). Rendered DOM shape, ARIA wiring, keyboard behavior, and the public API are unchanged. One placement note: non-`Tab` markup declared directly inside `<Tabs>` renders inside the `role="tablist"` element — declare non-tab content in a tab's own pane or as a sibling of `<Tabs>`.
