...
- **Skipped**: disabled fields (`aria-disabled="true"` counts — a disabled [`EditBool`](#editbool) with `AllowFocusWhenDisabled` on renders no `disabled` attribute at all), `readonly` fields *unless* they declare `aria-readonly="false"` (a non-searchable `EditSelectSearch`/`EditMultiSelect` does: its `readonly` only blocks typing, while the arrows, Enter, Space and type-ahead all still change the value), anything with `tabindex="-1"`, anything inside an `inert` or `aria-hidden` subtree, and anything not actually rendered or visible (a [`HidingMode`](#hidingmode)-hidden control, a `display:none`/`visibility:hidden` panel). Buttons and links aren't fields and are never chosen.
...
- **A form whose fields arrive later still gets focused.** When the scope contains no focusable field at all on its first render — the ordinary "load the model in `OnInitializedAsync`, render the form behind an `@if`" page — the attempt repeats on the scope's next render batches, up to **10 attempts** in total, and stops permanently at the first attempt that finds a candidate. A form present from the start is unaffected (it settles on attempt one), and a user who has already started typing is never interrupted, because a focused field counts as settled. For a strictly single attempt, put the `<FormDefaults>` *inside* the `@if` that renders the form, so its own first render is the form's:
  ```razor
  ...
  ```
- **Turning it on later doesn't arm it.** The scope is decided in `OnInitialized`, so flipping `FocusFirstField` from `false`/null to `true` after the component's first render is a permanent no-op — the same caveat as [`FocusOnFirstRender`](#programmatic-focus-focusasync--focusonfirstrender), and consistent with `FormDefaults` being set-once root configuration (its cascade is `IsFixed`). Render the scope with the value it should have, or re-mount it (`@key`).
- **Nested scopes: the outer one wins the tie.** A `FormDefaults` inherits `FocusFirstField` like every other setting, so `true` on an app root silently arms *every* nested `FormDefaults` underneath it — including one you added purely for `UpdateOn` or `AssetBase`. Set `FocusFirstField="false"` on an inner scope to opt it out. When two armed scopes first render in the *same* batch (an outer and an inner), the outer fires first and its choice stands, since the inner then finds a field already focused.
- **Needs `edit-controls.js`** (see [Quick Start](#quick-start)). Under prerender, static SSR, or with the script unreachable, focus simply doesn't move — no error. The scope requires no `IJSRuntime` registration either, so a `<FormDefaults>` still renders inside an out-of-band `HtmlRenderer` (email/PDF/static export) with or without the feature on.
- While the feature is on, the scope renders two empty `<template>` elements (one before and one after its content) to delimit itself for the DOM query. They have no layout box, no accessibility presence and carry nothing but an `id` — but they *are* elements, so **structural CSS in whatever contains your `<FormDefaults>` sees two more children**: `:first-child`/`:last-child`/`:only-child` stop matching (the end marker is genuinely the last child), `:nth-child` indices shift by one, the parent stops matching `:empty`, and `+`/`~` sibling combinators move. In a table, `tbody > tr:nth-child(odd)` zebra striping breaks — `:nth-of-type` doesn't, and is the safer selector generally. Flex and grid layout is unaffected (a `display:none` child is not a flex/grid item), and no library-internal styling is affected, because every positional selector this package ships is inside one control's own markup, which a `FormDefaults` can't split.
...
| `OnSaveFailed` | `EventCallback<FormAutoSaveFailureEventArgs>` | *(unset)* | Handles an exception from `OnSave`, and tells you which fields that save was carrying. When **not** wired, the exception is re-dispatched as if thrown from a lifecycle method — an enclosing `ErrorBoundary` catches it; failures are never silently swallowed (a handler that throws in turn is re-dispatched too, alongside the save failure it was handling). |
| `TimeProvider` | `TimeProvider?` | `null` | The clock the debounce runs on. Supply a fake one to test your form's auto-save without sleeping. Read **once**, when the debounce timer is first created — swapping it later has no effect. |
...
`FormAutoSaveFailureEventArgs` is `(EditContext EditContext, Exception Exception, IReadOnlyList<FieldIdentifier> ChangedFields)` — the same field list the failed save was handed. **Those fields are not lost:** they go back to the front of the pending set, so the next flush retries them ahead of anything that changed since and they reappear in that save's `ChangedFields`. A save that fails permanently therefore keeps re-attempting them on every later flush, which is what you want for a transient outage — and the reason you get the list is so you can do something else when it isn't (mark the fields unsaved, write a local draft, stop retrying).

Five things worth knowing before you wire it up:

- **Put it after `<DataAnnotationsValidator />`.** `EditContext.OnFieldChanged` handlers run in subscription order, and the `SaveWhenInvalid="false"` gate reads the context's *current* validation messages (it deliberately does not call `Validate()`, which would light up every untouched field's error the moment the user typed in one). The ordering only actually *bites* at `DebounceMilliseconds="0"`, where the flush runs inside the notification itself: put the component first there and the gate reads a store the validator hasn't touched yet, saving the invalid value. At any non-zero debounce the flush happens a debounce later, long after every handler has run. Write it after the validator anyway — it costs nothing and it's correct in both modes.
- **A skipped save is never terminal.** It keeps its pending fields, so once the form is valid again the next save still reports everything that changed in the meantime — and "valid again" doesn't have to mean another field change: the component also re-attempts on `EditContext.OnValidationStateChanged`, so clearing a `ValidationMessageStore` server-side and calling `NotifyValidationStateChanged()` releases the pending work. Without that, a field that simply stays invalid (a conditionally-required property the user can't currently satisfy) would gate off auto-save for the entire form.
...
`FormAutoSave` renders nothing (the form's DOM is byte-identical with and without it) and disposes its subscriptions with the form. It also sees changes a per-field `:after` cannot: `EditFile`'s remove button, for one, mutates the bound list without any field setter running.
...
- **`Table<TItem>`** — Data table with `Column`/`PropertyColumn`/`ActionColumn`, row selection, paging (pager placement via `PagerPosition` = Top/Bottom/Both and alignment via `PagerAlign`), and column sorting (`Sortable="true"` on a `PropertyColumn` — non-comparable types degrade to non-sortable; or a `SortBy` comparison on any column). Columns may be conditionally rendered (`@if`).
  ...
  - `Loading`, `IsRowSelectable`, `SelectionMode.Single`, controlled expansion (`ExpandedRowKeys`/`OnExpand`), `ExpandRowByClick`/`OnRowClick`, `Column.Ellipsis`, `EmptyContent`, `FooterContent`, column filtering (multiple kinds, dropdown or row placement, optional server-side), and `ScrollY` (a scrollable body with a sticky header) round out AntD 4.x parity. See [Table parity features](#table-parity-features-tabletitem).
...
- **Column filtering: `Column.FilterOptions`/`OnFilter`/`FilterMultiple`** — a funnel icon appears in the header cell (after the sort control, when both are present — clicking it never triggers a sort) whenever a column sets `FilterOptions` (`IReadOnlyList<TableFilterOption>`, each a `Text`/`Value` pair) **and** `OnFilter` (`Func<TItem, string, bool>`, given one selected value); either alone renders no filter UI.
  ...
  ```
- **More filter kinds: `Column.FilterText`, `Column.FilterDropdown`** — alongside `FilterOptions`+`OnFilter`. `FilterText` (`Func<TItem, string?>`) adds a search box; `TextFilterMatch` (`Contains` default, `StartsWith`, `Equals`, all ordinal-ignore-case) sets the comparison. `FilterDropdown` (`RenderFragment<TableFilterContext<TItem>>`) replaces the whole panel with your own template — stage with `SetSelectedValues`, apply with `ConfirmAsync()`, clear with `ResetAsync()`. `FilterIcon` swaps the funnel glyph; `OnFilterDropdownOpenChange` reports every open/close; `FilterOnClose` makes dismissing the dropdown commit instead of discard. Kind precedence: an explicit `FilterDropdown` / `FilterOptions`+`OnFilter` / `FilterText` always wins over `Filterable`/`FilterValuesFromData` below, and a `DataSource` swap never touches an explicitly declared filter.
- **`PropertyColumn.Filterable`/`FilterValuesFromData`** — derive a filter from the property type instead of declaring one, the way `Sortable` derives its comparison: `string` → text box, a numeric type → inclusive min/max, `DateTime`/`DateOnly`/`DateTimeOffset` → inclusive date range, `bool` → yes/no/any, `enum` → member checkbox/radio list. `FilterValuesFromData` builds the option list from the distinct values actually in `DataSource` instead — no funnel until that list has at least one option (an empty/null `DataSource` renders none), `DefaultFilterValues` applies on the pass options first exist, and the options re-derive on a culture change.
  ```razor
  <PropertyColumn TItem="Product" TProp="decimal" Title="Price" Property="@(p => p.Price)" Format="C2" Filterable="true" />
  ```
- **`Table.FilterPlacement="TableFilterPlacement.Row"`** — an always-visible editor per filterable column in a second header row, instead of funnel dropdowns. Text editors debounce (`FilterDebounceMilliseconds`, default 300ms); selections apply immediately. Renders no filter row at all while no column is filterable.
- **`Table.OnFiltersChanged`** — the aggregate counterpart of `OnFilterChanged`: one `IReadOnlyList<TableColumnFilterSnapshot<TItem>>` of every currently-active column, handy for a "clear all" summary. **`ClearFiltersAsync()`** resets every column at once, also closing any open dropdown, including one with nothing applied.
- **Dropdown extras** — `FilterSearch` (search box over a long option list), `FilterCheckAll` ("select all", respecting the search; multi-select Options columns only), `DefaultFilterValues`/`FilterResetToDefault` (initial filter, and what Reset returns to instead of clearing — a single-select column keeps only the first key of a multi-key `DefaultFilterValues`).
- **Filter strings** — every visible filter label/format is a `Table` parameter: `FilterRowLabelFormat` (`"Filter by {0}"`), `FilterResultAnnouncementFormat` (`"{0} matching rows"`), `FilterClearLabel` (`"Clear"`), `FilterMinLabel`/`FilterMaxLabel` (`"Minimum"`/`"Maximum"`), `FilterBoolAnyText`/`FilterBoolTrueText`/`FilterBoolFalseText` (`"All"`/`"Yes"`/`"No"`), `FilterSelectPlaceholder` (`"All"`, the `Row`-placement Options `Select`), `FilterSearchPlaceholder` (`"Search in filters"`), `FilterEmptyText` (`"No matches"`, also that `Select`'s no-match text), `FilterCheckAllLabel` (`"Select all"`), `FilterSelectedCountFormat` (`"{0} selected"`), `FilterContainsFormat`/`FilterStartsWithFormat`/`FilterEqualsFormat` (`contains "{0}"` etc., drive `TableColumnFilterSnapshot.Description`), `FilterDateStartLabel`/`FilterDateEndLabel` (`"Start date"`/`"End date"`).
- **`Table.ClientSideFiltering="false"`** — keeps the filter UI/events but stops the `Table` narrowing rows itself, for server-side filtering: handle `OnFiltersChanged`, fetch, reassign `DataSource`.
- **The dropdown escapes the table's overflow clip** — it switches to `position: fixed` (JS) on *every* open, not just under `ScrollY`: `.wss-table-wrapper`'s `overflow-x: auto` computes `overflow-y: auto` too, so even a three-row table clipped a panel. It tracks its trigger across scroll/resize while open; without JS it stays CSS-anchored and may clip.
- **Enter** in a filter text/search/number box applies the filter — in a dropdown it also closes the panel, like OK — without submitting an enclosing `<EditForm>` (suppressed in JS; with no JS runtime the form submits alongside it).
- One rough edge, left as-is: a `Row`-placement `Select`/date-range popup can get clipped by the table wrapper (worst under `ScrollY`) — only the funnel dropdown escapes it.
- **`ScrollY` (`string?`)** — bounds the table body to a fixed height with its own vertical scrollbar and a sticky header (AntD's `scroll.y` equivalent): any CSS length (`"320px"`). Null (default) renders the existing unconstrained wrapper.
  ...
  - An open column filter dropdown escapes the wrapper's overflow clip via `position: fixed` (see the filter bullets above, where that now happens on every open) and paints above `Loading`'s mask even while both are active together.
  ```razor
...
`Pagination` is a controlled component (`Total` / `Current` / `PageSize` + `CurrentChanged`), so it shows the correct page count from the server total and raises `CurrentChanged` when the user picks a page. Handle **sorting** the same way — pass the sort field/direction into your request rather than using `Sortable`, which only orders the page already loaded. For **filtering**, set `Table.ClientSideFiltering="false"` (see [Table parity features](#table-parity-features-tabletitem)) instead of fighting the built-in narrowing. A runnable paging example (with a simulated server) is in the `/uikit` gallery.
...
- **Don't put a plain `[Required]` on the OtherValue property.** The notification means the validator now runs against that property on *every* write — including the null the control writes when you switch away from Other. A `[Required]` OtherValue therefore reports "required" for as long as a non-Other option is selected: a real error on a field the user can't fill, and with `FormAutoSave`'s default `SaveWhenInvalid="false"` it gates the whole form's auto-save (recoverable — the skip is re-attempted when validation state changes — but it won't clear itself). The property is *conditionally* required, so express that: a `RequiredIf`-style attribute, `IValidatableObject`, a FluentValidation `When` rule, or `FormOptions.RequiredResolver` for the star/`aria-required` half.
- **The registration follows what actually renders.** The free-text box exists only in edit mode and only while `HasOtherOption` is set: in read-only mode the field's summary link moves onto the read-only element (which shows the `"Other: text"`), and with `HasOtherOption="false"` the field isn't registered at all. Both conditions are tracked at runtime, so a link never points at an element that isn't there.
...
**Name collisions.** Across every single-editor control the rule is: **the control wins when it has an opinion; your value survives when it doesn't.** So `<EditString @bind-Value="m.Name" disabled />` really does render a disabled input even though `IsDisabled` is left at its default, and a hand-written `aria-required` / `aria-errormessage` / `list` survives on a field where the control has nothing to say — while `IsDisabled="true"`, a `[Required]` property, or a real validation error still wins outright. This holds for `EditString`, `EditNumber`, `EditTextArea`, `EditDateNative`, `EditSelect`, `EditSelectEnum`, `EditSelectString`, `EditBool` and `EditRange`. Four qualifications:

- **`type` is control-owned outright** on `EditString`/`EditNumber`, in every state, and a splatted one is dropped rather than honored. `type` doesn't describe the element, it decides what the element *is* — and on `EditString` it is load-bearing for the whole `IsPassword` bundle (the reveal toggle, the bullet-masked read-only row, the redacted `ShowBoundValues` echo, the `new-password` autocomplete, the `Suggestions` suppression), which a splatted `type="password"` would half-implement. Reach the same goals through the supported channels: `inputmode` (splats through untouched — this is the one for mobile soft keyboards), `Autocomplete`/`[Autocomplete]` for the field's purpose, and `IsPassword`/`[DataType(Password)]` for a secret.
...
- **`EditBool`'s `disabled`/`aria-disabled` pair has its own shape.** `AllowFocusWhenDisabled` (default `true`) deliberately withholds the native `disabled` attribute while the checkbox is non-operable, so it stays a real Tab stop — a splatted `disabled` can't reintroduce it while that opt-in is on, and this wins outright the same way an ordinary "control has an opinion" case does. Set `AllowFocusWhenDisabled="false"` for fully-native behavior, where `disabled` collisions resolve exactly like every other control.
- **`EditRange`'s field element is a `role="slider"` `<div>`**, which has no native `disabled` attribute at all — it uses `aria-disabled` in that slot instead, following the same collision rule as everywhere else.

Not every control participates in this: some genuinely never put your splatted attributes on the same element that carries their own conditional state, so there's no collision to resolve in the first place. `EditFile`'s `<InputFile>`, each checkbox inside `EditCheckedEnumList`/`EditCheckedStringList`, and each option inside `EditRadio`/`EditRadioEnum`/`EditRadioString`/`EditBoolNullRadio` all leave your unmatched attributes on an outer wrapper (or don't capture them at all) rather than on that per-item element. Same reasoning covers the two Select-engine controls (`EditSelectSearch`/`EditMultiSelect`) and the calendar pickers (`EditDate`/`EditDateRange`): their inner field's `disabled`/`aria-*` come from the parent control's own resolved state, passed down as ordinary component parameters rather than merged into a shared attribute dictionary.
...
- Every filter label/format is a `Table` parameter — see [Table parity features](#table-parity-features-tabletitem) for the full list. **`FilterResultAnnouncementFormat`** (default `"{0} matching rows"`) is new: a `role="status"` region now announces it after every filter change, so an existing `FilterOptions`/`OnFilter` column gains an announcement it didn't have before — set it to `""` to silence (a null value throws).
...
- **`TableColumnFilter<TItem>`** lost its own `FilterButtonLabelFormat`, `FilterAppliedButtonLabelFormat`, `FilterAppliedLabel`, `FilterLabel` parameters — the accessible name now comes from `Table`'s parameters of the same names. Only affects code that instantiates `TableColumnFilter` directly; the header funnel `Table` itself renders is unaffected.
- **`PropertyColumn<TItem,TProp>`** now annotates `TProp` with `[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.All)]`. Concrete `TProp="..."` markup is unaffected; a consumer generic that forwards its own unannotated `TProp` into `PropertyColumn` needs the same annotation to avoid IL2091 under trim analysis.
...
An additive, non-breaking release: two new form-level helpers (`FormAutoSave`, `FormDefaults.FocusFirstField`), programmatic and declarative focus (`FocusAsync()`/`FocusOnFirstRender`) on every control, two new field-level parameters (`Suggestions`, `LabelContent`), a two-way-bindable `EditRadioEnum` "Other" field, and two attribute-handling contracts — event-handler chaining and the null-attribute-erasure fix — that previously covered only a handful of controls now covering every one that can.

**New**
- **`FormAutoSave`** — one component that replaces a form's per-field `@bind-Value:after="SaveAsync"` wiring. Placed once inside the `EditForm`, it subscribes to the shared `EditContext` and debounces (`DebounceMilliseconds`, default 500ms trailing) every field's changes into one save, calling your required `OnSave` with `FormAutoSaveEventArgs` (`EditContext`, and `ChangedFields` — the de-duplicated, first-seen-order field list since the last save). `SaveWhenInvalid` (default false) gates saving while the form has outstanding validation messages, without losing the pending fields — the next successful save (or the next `OnValidationStateChanged`, so a server-side error clear also releases it) still reports everything that changed in the meantime. `Concurrency` (`CoalesceTrailing` default, or `Concurrent`) controls what happens when changes arrive mid-save; `ShouldSave` is a per-field filter; `TimeProvider` lets a test drive the debounce without sleeping. New `OnSaveFailed` (`EventCallback<FormAutoSaveFailureEventArgs>`) handles an exception from `OnSave` and hands back the fields that save was carrying — **which go back to the front of the pending set** rather than being dropped, so a transient failure keeps retrying them on every later flush instead of silently losing the edit; unwired, the exception re-dispatches for an `ErrorBoundary` to catch, so a failure is never swallowed either way. `FormAutoSave` renders nothing (byte-identical form DOM) and sees changes a per-field `:after` cannot, like `EditFile`'s remove button mutating the bound list with no setter running. See [`FormAutoSave`](#formautosave).
- **`FormDefaults.FocusFirstField`** (`bool?`, default off) — focuses the first field rendered beneath a `FormDefaults` scope, once, after that scope's first render: the form-level counterpart to a single control's `FocusOnFirstRender`, set once at the form or dialog instead of on whichever control happens to be written first. Resolved in the **rendered DOM** (so it follows markup order, not construction order), it skips disabled fields, `readonly` fields unless they declare `aria-readonly="false"`, `tabindex="-1"` elements, and anything hidden or inert; an explicit `FocusOnFirstRender` on a specific control still wins the race; a form whose fields arrive asynchronously gets a bounded retry (up to 10 render batches) rather than a single missed attempt; and nested `FormDefaults` scopes resolve outer-wins-the-tie. Needs `edit-controls.js` and degrades to a no-op without it (prerender, unreachable script) — no `IJSRuntime` registration is required either, so a scope still renders under an out-of-band `HtmlRenderer` (email/PDF/static export) with the feature on or off. See [`FocusFirstField`](#focusfirstfield).
- **Public `FocusAsync()` and `FocusOnFirstRender`** on every `Edit*` control — `FocusAsync()` (a `ValueTask`, held via `@ref`) programmatically focuses whichever element a `Tab` into the control would land on (the checked radio/first enabled checkbox for the groups, the Start input for `EditDateRange`, the swatch trigger — not the popup — for `EditColor`, etc.); `FocusOnFirstRender` (`bool`, default false) is its declarative form, focusing the control once after its first render. Neither ever moves focus to a control the user couldn't reach anyway — disabled, read-only-mode, hidden, or not yet interactive are all silently no-ops, so call sites don't need to guard on state they can't see. `EditBool` is the one disabled control that still takes focus, matching its existing `AllowFocusWhenDisabled` opt-in. The radio groups and checked lists resolve their target via `edit-controls.js` (no element reference is capturable through `InputRadio`) and degrade to a no-op without it, same as everywhere else in the library. Inside a `Modal`/`Drawer`, a child's own `FocusOnFirstRender`/`FocusAsync()` now wins the initial-focus race against the dialog's own close-button default. **The parameter is spelled `FocusOnFirstRender`, not `AutoFocus`, deliberately** — Blazor matches parameter names case-insensitively, so an `AutoFocus` parameter would have captured the native `autofocus` attribute instead of letting it splat through. See [Programmatic focus](#programmatic-focus-focusasync--focusonfirstrender).
- **`Suggestions`** (`IEnumerable<string>?`) on `EditString` and `EditNumber` — renders an HTML `<datalist>` wired to the input's `list` attribute, for **open**-vocabulary autofill hints (any typed value is still accepted), unlike a Select control's closed vocabulary. Null renders neither the attribute nor the list (a hand-wired `list=`/`<datalist>` keeps working untouched); an empty sequence still renders both, so a filtering fetch doesn't flicker the attribute. Re-enumerated every render (pass something stable), suppressed on a password field even after the reveal toggle, and generates its own per-instance `dl-{guid}` id so a list of rows bound to the same property never cross-wires suggestions between rows. See [`EditString`](#input-controls).
- **`LabelContent`** (`RenderFragment?`) on every control — a rich-markup alternative to `Label` for content a plain string can't hold (an icon plus text in a toggle-group row, for example), replacing the label text everywhere `Label` would render, including `EditBool`'s checkbox row. Phrasing content only — no nested interactive elements, since accessible-name computation would fold a descendant control's own name into the field's name. Still set `Label` (or a meaningful property name/`[DisplayName]`) alongside it: validation text and the accessible-name fallback chain read `Label`, never the `LabelContent` fragment. See [Rich-markup labels](#rich-markup-labels-labelcontent).
- **`EditRadioEnum<TEnum>`: bind the "Other" free-text box with `@bind-OtherValue`** — two-way binding the second model property gives it a real `FieldIdentifier`, registers it under the box's own element id (`other-{id}`), and calls `NotifyFieldChanged` on every write (typing, the clear on switching away from Other, the re-commit on switching back), enrolling it in the validation summary along the way. The registration tracks what's actually rendered — edit mode only, and only while `HasOtherOption` is set — so a summary link never dangles at an element that isn't there. Wiring `OtherValue`/`OtherValueChanged` separately instead leaves the old, unregistered behavior untouched. Don't put a plain `[Required]` on the bound property — the notification means the validator runs on every write, including the null the control writes while a non-Other option is selected — use a conditional-required pattern instead. See [The "Other" text box is a real field](#the-other-text-box-is-a-real-field--bind-it-with-bind-othervalue).

**Changed**
- **The event-handler chaining contract.** A `@onclick`/`@onkeydown`/`@onmousedown` you splat onto a control that already binds a handler of that same name on that same element — `EditRange`'s slider track, both date pickers' wrappers, `Modal`/`Drawer`'s dialog panel, and the `Select` engine's wrapper — previously had one handler silently discard the other. The two now **chain**: the control's own behavior runs first, then yours; yours still runs even when the control's own handler does nothing (a disabled `EditRange`, a `Modal` with `Keyboard="false"`); and you're never cut off by the control's own `stopPropagation`, since your handler now shares its element instead of sitting on a wrapper outside it. `EditDate<T>`, `EditDateRange`, and `EditSelectSearch<TValue>` forward their whole splat into the same chain. Two exceptions remain deliberate: the binding event itself (`oninput`/`onchange`) still belongs to the control, and an *open* popup (`Select`, `ColorPicker`/`EditColor`) still owns the keyboard on its inner element rather than the wrapper you splat onto. See [Your own event handlers](#your-own-event-handlers-onkeydown-onclick-).
- **The null-attribute-erasure fix now covers every single-editor control, not just `EditString`/`EditNumber`.** The rule — the control wins when it has an opinion on an attribute, your splatted value survives when it doesn't — now also holds for `EditTextArea`, `EditDateNative`, `EditSelect`/`EditSelectEnum`/`EditSelectString`, `EditBool` (respecting its `AllowFocusWhenDisabled` opt-in), and `EditRange` (whose `role="slider"` div uses `aria-disabled` in the slot a native `disabled` would occupy elsewhere). Previously, a splatted `disabled`/`aria-required`/`aria-errormessage`/`list` on any of those controls was dropped outright whenever the control's own value for that attribute was null, even though the identical attribute already survived on `EditString`/`EditNumber`. See "Name collisions" under [Opt-in AntD theme for the classic edit inputs](#opt-in-antd-theme-for-the-classic-edit-inputs-edit-theme).

**Fixed**
- **Every control's read-only rendered value now carries `AriaDescribedBy`**, wiring in the description/tooltip/error-message ids its editor already carries — `EditBool`, `EditBoolNullRadio`, `EditDate<T>`, `EditDateNative<T>`, `EditDateRange`, `EditRadio<T>`, `EditRadioEnum<TEnum>`, `EditRadioString`, `EditSelect`, `EditSelectEnum`, `EditSelectString`, `EditMultiSelect<T>`, and `EditSelectSearch<T>`. A read-only field previously announced only its bare value, dropping its description, tooltip text, and any validation message — precisely the information a reader has no other way to reach once the editor is gone.
- **`EditCheckedEnumList`/`EditCheckedStringList`'s read-only fieldset is now nameable.** Its `aria-labelledby` was previously conditional on edit mode (alongside the `id`/`role="group"` that genuinely only apply there); read-only mode's `<fieldset>` still maps to `role="group"` natively, so it silently fell back to its `<legend>` for a name — which also contains the `LabelTooltip` trigger, folding "More information about X" into the announced name. The reference is now unconditional, pointing at the label's own text anchor in both modes.
- **`EditColor`'s read-only row is now legitimately nameable.** Its `aria-labelledby` sat on a bare, role-generic `<div>`, where ARIA prohibits naming (axe-core's `aria-prohibited-attr`, and some assistive technology drops the name entirely) — it now renders `role="group"`, accurate since the row genuinely groups the swatch and, under `ShowText`, the hex string.
- **`ToastQueue`**: an intermittent `ObjectDisposedException` out of `Pause`/`Resume` — publishing a new timer's `CancellationTokenSource` before reading its `Token` let a concurrent `Pause`/`Remove`/`Clear` cancel-and-dispose it first, and reading `.Token` after disposal throws. The token is now captured before publishing.
- **`FormDefaults` no longer requires an `IJSRuntime` registration just to exist in a tree.** It resolved `IJSRuntime` via `[Inject]` — which throws when the service is missing — purely to support the (default-off) `FocusFirstField` feature, making any `<FormDefaults>` a hard dependency even where nothing asked for focus-moving; a `HtmlRenderer` over a minimal service provider (email templates, PDF pipelines, static HTML export) threw outright. It now resolves `IJSRuntime` optionally at call time; a missing service just means no focus move, matching the rest of the library's best-effort JS-interop contract.
- **`AttributeSplat.RestExcept`**: a case-variant withheld attribute (`onKeyDown`, `aria-Label`) was detected as present (the presence check already matched case-insensitively, like Blazor's own splat dictionary) but never actually removed (the removal loop compared case-sensitively) — concretely, a case-variant `aria-Label` leaked a duplicate copy onto `Select`'s roleless wrapper alongside the one correctly re-homed onto its combobox input. The removal loop now matches the same comparer as the presence check.
