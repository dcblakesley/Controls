...
- List-bound controls (`EditMultiSelect`, `EditFile`, `EditCheckedStringList`, `EditCheckedEnumList`): a `class` attribute is now captured and merged into the rendered field, `InputBase.CssClass`-style, instead of throwing at render time as an unmatched parameter. In particular `EditMultiSelect` now forwards the consumer's `class` to the select engine, matching `EditSelectSearch`.
